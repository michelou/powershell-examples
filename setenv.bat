...
@rem input parameters: %1=use_claude %2=verbose
:print_env
set __USE_CLAUDE=%~1
set __VERBOSE=%~2
...
if %ERRORLEVEL%==0 (
    set __POWERSHELL_VERSION=
    for /f "tokens=*" %%i in ('powershell.exe -c "(Get-Host).Version.ToString()"') do (
         if not defined __POWERSHELL_VERSION set "__POWERSHELL_VERSION=%%i"
    )
    set "__VERSIONS_LINE1=%__VERSIONS_LINE1% powershell !__POWERSHELL_VERSION!,"
)
where /q pwsh.exe
if %ERRORLEVEL%==0 (
    set __PWSH_VERSION=
    for /f "tokens=1,2,*" %%i in ('pwsh.exe --version') do (
         if not defined __PWSH_VERSION set "__PWSH_VERSION=%%j"
    )
    set "__VERSIONS_LINE1=%__VERSIONS_LINE1% pwsh !__PWSH_VERSION!,"
)
where /q "%GIT_HOME%\bin:git.exe"
if %ERRORLEVEL%==0 (
    for /f "tokens=1,2,*" %%i in ('call "%GIT_HOME%\bin\git.exe" --version') do (
...
endlocal & (
    if %_EXITCODE%==0 (
        ...
        set "PATH=%_GIT_HOME%\bin;%PATH%%_CLAUDE_PATH%%_GIT_PATH%%_VSCODE_PATH%;%~dp0bin"
